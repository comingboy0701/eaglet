...
class ATModel(base_model.ATModel):
    """
    ...
    def train(self, trains, labels, optimizer,  *args, **kwargs):
        """
        ...
        # 先计算原始样本下的梯度
        outputs, loss = self.model(trains, labels=labels)           # 前向传播，计算分类得分
        self.model.zero_grad()                                      # 清除梯度
        # loss = loss_fun(outputs, labels)                          # 交叉熵loss
        ...
        for step in range(self.K):
            ...
            outputs, loss = self.model(trains, labels=labels)       # 重新前向计算一次得分，并计算对抗样本的损失
